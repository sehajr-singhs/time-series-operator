...
curve. The matched-compute experiment settles the direction of that
gap: fine-tuning Chronos on the operator's own corpus at equal
parameter-steps leaves it unchanged (21W/19L vs the true frozen
baseline, $p=0.98$) and even at $22\times$ the budget gains only
modestly, not significantly ($p=0.20$), so
pretraining scale --- not tokenization --- currently decides the
head-to-head (Chronos 24--29W/16--11L over the TSO,
$p<0.01$). Ablations show
...
\paragraph{The matched-compute experiment.}
The scale explanation makes a falsifiable prediction: give Chronos the
\emph{identical} TSO training corpus at \emph{equal} compute, and the gap
should collapse if it is purely data. We tested this directly
(Fig.~\ref{fig:matched}): chronos-t5-small (46.2M parameters) was
fine-tuned on the exact v14 corpus (232 series, 496 pool entries) at the
parameter-matched budget --- 1{,}127 steps, equal parameter-steps to the
TSO's $2.08\text{M} \times 25{,}000$ --- and at a generous equal-step
budget of 25{,}000 steps ($22\times$ the matched budget), using the
publisher's own tokenizer and training objective. The prediction fails:
fine-tuning at the matched budget does not improve Chronos at all
(21W/19L vs the true frozen baseline, $p=0.98$, two-sided Wilcoxon;
median skill $+22.7$ vs $+21.4$ on the identical protocol), and even the
$22\times$-budget fine-tune gains only modestly, not significantly
(22W/18L, $p=0.20$; median $+33.3$ vs $+21.4$). (One methodological
note: the kernel's in-run ``frozen'' column was contaminated --- its
frozen pipe shared the model object that fine-tuning mutated in place,
correlating 0.986 with the generous column --- so the true frozen
baseline above was re-measured post-hoc on the byte-identical protocol;
this strengthens rather than weakens the null.) Against the TSO, all
three Chronos variants retain the frozen model's advantage (24W/16L
frozen, 27W/13L matched at $p=0.006$, 29W/11L generous). The
corpus is simply too small to move a model already pretrained on
\textasciitilde 80{,}000 series; Chronos's dominance is bought upstream,
not on the training task. The experiment therefore does \emph{not}
support a paradigm claim at matched compute --- the honest conclusion is
that pretraining scale, not tokenization, currently decides the
head-to-head --- and it delimits precisely what the TSO's architecture
must earn: a corpus of comparable breadth, on which the operator
machinery (linearizable latent, scale covariance, phase-space geometry)
is untested against an incumbent fine-tuned on the same data.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{fig_matched.png}
\caption{The matched-compute experiment (v18): TSO v14 (frozen, 2.1M
params, 25k iterations) vs chronos-t5-small (46.2M params) frozen and
fine-tuned on the \emph{identical} corpus at parameter-matched steps
(1{,}127; equal parameter-steps to the TSO) and at generous equal steps
(25k, $22\times$ the matched budget). Bars are skill vs persistence per
series on the shared 40-series protocol; medians in the legend.
Fine-tuning does not move Chronos at either budget, and both checkpoints
retain the frozen model's advantage over the TSO: pretraining scale,
not tokenization, currently decides the comparison.}
\label{fig:matched}
\end{figure}
